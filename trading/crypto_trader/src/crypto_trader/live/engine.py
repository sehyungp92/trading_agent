...
_SIDECAR_BLOCK_REASON = "assistant relay sidecar unhealthy"
_SIDECAR_INCIDENT_ACTION = "cancel_working_entry_orders"
_SIDECAR_FAILURE_BLOCK_THRESHOLD = 3
_SIDECAR_BACKLOG_BLOCK_THRESHOLD = 500
_SIDECAR_BACKLOG_AGE_BLOCK_SECONDS = 900.0
...
class LiveEngine:
    ...
    def _dispatch_bar(self, bar: Bar | MarketEvent) -> None:
        ...
        self._refresh_sidecar_runtime_policy()
        for slot in self._slots:
            if (
    ...
    async def _health_report_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                sidecar_status = self._refresh_sidecar_runtime_policy()
    ...
    def _refresh_sidecar_runtime_policy(self) -> dict:
        sidecar_status = self._relay_health_status()
        self._apply_sidecar_runtime_policy(sidecar_status)
    ...
    def _relay_health_status(self) -> dict:
        ...
        if self._sidecar is None:
            ...
            status["runtime_policy"] = self._sidecar_runtime_policy(status)
            return status
        ...
        return {
            ...
            "buffered_event_count": status.get("buffered_event_count", 0),
            "oldest_buffered_event_age_seconds": status.get(
                "oldest_buffered_event_age_seconds",
                0.0,
            ),
            ...
            "runtime_policy": self._sidecar_runtime_policy(status),
        }

    def _sidecar_runtime_policy(self, status: dict) -> dict[str, Any]:
        """Return entry standdown policy derived from sidecar delivery health."""
        reasons: list[str] = []
        if status.get("enabled") is False:
            reasons.append("relay_sidecar_not_configured")
        elif not bool(status.get("running", False)):
            reasons.append("sidecar_not_running")
        failures = int(status.get("consecutive_send_failures") or 0)
        if failures >= _SIDECAR_FAILURE_BLOCK_THRESHOLD:
            reasons.append(f"consecutive_send_failures>={_SIDECAR_FAILURE_BLOCK_THRESHOLD}")
        buffered = int(status.get("buffered_event_count") or 0)
        if buffered >= _SIDECAR_BACKLOG_BLOCK_THRESHOLD:
            reasons.append(f"buffered_event_count>={_SIDECAR_BACKLOG_BLOCK_THRESHOLD}")
        oldest_age = float(status.get("oldest_buffered_event_age_seconds") or 0.0)
        if oldest_age >= _SIDECAR_BACKLOG_AGE_BLOCK_SECONDS:
            reasons.append(f"oldest_buffered_event_age_seconds>={_SIDECAR_BACKLOG_AGE_BLOCK_SECONDS:g}")
        return {
            "ok": not reasons,
            "standdown_required": bool(reasons),
            "standdown_reason": _SIDECAR_BLOCK_REASON if reasons else "",
            "incident_action": _SIDECAR_INCIDENT_ACTION if reasons else "none",
            "incident_action_scope": "strategy_entry_orders_only" if reasons else "none",
            "open_position_action": "hold_existing_positions" if reasons else "none",
            "reasons": reasons,
            "thresholds": {
                "consecutive_send_failures": _SIDECAR_FAILURE_BLOCK_THRESHOLD,
                "buffered_event_count": _SIDECAR_BACKLOG_BLOCK_THRESHOLD,
                "oldest_buffered_event_age_seconds": _SIDECAR_BACKLOG_AGE_BLOCK_SECONDS,
            },
        }

    def _apply_sidecar_runtime_policy(self, sidecar_status: dict) -> None:
        manager = getattr(self, "_manager", None)
        if manager is None:
            return
        policy = sidecar_status.get("runtime_policy")
        if not isinstance(policy, dict):
            policy = self._sidecar_runtime_policy(sidecar_status)
            sidecar_status["runtime_policy"] = policy
        current_reason = str(getattr(manager, "entries_blocked_reason", "") or "")
        if policy.get("standdown_required"):
            if current_reason in {"", _SIDECAR_BLOCK_REASON}:
                manager.entries_blocked_reason = _SIDECAR_BLOCK_REASON
            if policy.get("incident_action") == _SIDECAR_INCIDENT_ACTION:
                policy["incident_action_applied"] = True
                policy["entry_orders_cancelled"] = (
                    self._cancel_working_entry_orders_for_sidecar_incident()
                )
            return
        if current_reason == _SIDECAR_BLOCK_REASON:
            manager.entries_blocked_reason = ""

    def _cancel_working_entry_orders_for_sidecar_incident(self) -> int:
        cancelled = 0
        for slot in getattr(self, "_slots", []):
            broker = getattr(getattr(slot, "ctx", None), "broker", None)
            if broker is None:
                continue
            try:
                orders = list(broker.get_open_orders(""))
            except Exception:
                log.exception("engine.sidecar_incident_open_order_query_failed")
                continue
            for order in orders:
                metadata = getattr(order, "metadata", {}) or {}
                role = str(
                    getattr(order, "tag", "") or metadata.get("role") or metadata.get("tag") or ""
                ).lower()
                if role != "entry":
                    continue
                order_id = str(getattr(order, "order_id", "") or "")
                try:
                    if order_id and broker.cancel_order(order_id):
                        cancelled += 1
                except Exception:
                    log.exception("engine.sidecar_incident_entry_cancel_failed", order_id=order_id)
        return cancelled
