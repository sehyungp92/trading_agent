...
class TestLiveConfigRelayPlumbing:
    def test_round_trips_optional_bot_and_relay_fields(self, tmp_path):
        cfg = LiveConfig.from_dict({
            ...
            "relay_secret": "0123456789abcdef0123456789ABCDEF",
        })

        data = cfg.to_dict()
        assert data["bot_id"] == "paper_bot_01"
        assert data["relay_url"] == "https://relay.example.com"
        assert data["relay_secret"] == "0123456789abcdef0123456789ABCDEF"
    ...
    def test_validate_rejects_placeholder_low_entropy_and_loopback_relay(self):
        cfg = LiveConfig(
    ...
    def test_validate_requires_relay_when_paper_live_runtime_demands_it(self):
        cfg = LiveConfig(
            wallet_address="0x" + "1" * 40,
            private_key="0x" + "2" * 64,
            symbols=["BTC"],
        ...
        errors = cfg.validate(require_relay=True)

        assert "relay_url is required when relay is configured" in errors
        assert "relay_secret is required when relay is configured" in errors
...
class TestLiveHealthRelayStatus:
    ...
    def test_relay_health_status_reports_disabled_without_sidecar(self):
        ...
        assert engine._relay_health_status() == {
            ...
            "runtime_policy": {
                ...
                "standdown_reason": "assistant relay sidecar unhealthy",
                "incident_action": "cancel_working_entry_orders",
                "incident_action_scope": "strategy_entry_orders_only",
                "open_position_action": "hold_existing_positions",
                "reasons": ["relay_sidecar_not_configured"],
                "thresholds": {
                    "consecutive_send_failures": 3,
                    "buffered_event_count": 500,
                    "oldest_buffered_event_age_seconds": 900.0,
                },
            },
        }

    def test_missing_relay_sidecar_explicitly_blocks_new_entries(self):
        from crypto_trader.live.engine import LiveEngine

        engine = object.__new__(LiveEngine)
        engine._sidecar = None
        engine._manager = SimpleNamespace(entries_blocked_reason="")
        engine._slots = []
        status = engine._relay_health_status()

        engine._apply_sidecar_runtime_policy(status)

        assert engine._manager.entries_blocked_reason == "assistant relay sidecar unhealthy"
        assert status["runtime_policy"]["incident_action_applied"] is True
        assert status["runtime_policy"]["entry_orders_cancelled"] == 0

    def test_dispatch_bar_refreshes_sidecar_policy_before_strategy_runtime(self):
        from crypto_trader.live.engine import LiveEngine

        engine = object.__new__(LiveEngine)
        engine._sidecar = None
        engine._manager = SimpleNamespace(entries_blocked_reason="")
        engine._health = SimpleNamespace(on_bar_received=MagicMock())
        runtime = MagicMock()

        def process_bar(*_args, **_kwargs):
            assert engine._manager.entries_blocked_reason == "assistant relay sidecar unhealthy"

        runtime.process_bar.side_effect = process_bar
        engine._slots = [
            SimpleNamespace(
                subscribed_tfs={TimeFrame.M15},
                strategy=SimpleNamespace(symbols=["BTC"]),
                ctx=SimpleNamespace(broker=SimpleNamespace()),
        ...
        runtime.process_bar.assert_called_once()
    ...
    def test_relay_health_status_maps_sidecar_fields(self):
        ...
        sidecar.status.return_value = {
            ...
            "oldest_buffered_event_age_seconds": 120.0,
        ...
        assert status["oldest_buffered_event_age_seconds"] == 120.0
        assert status["runtime_policy"]["ok"] is True
    ...
    def test_sidecar_runtime_policy_blocks_and_clears_new_entries(self):
        from crypto_trader.live.engine import LiveEngine
        ...
        unhealthy = {
            ...
            "oldest_buffered_event_age_seconds": 0.0,
        }
        status = {"runtime_policy": engine._sidecar_runtime_policy(unhealthy)}
        engine._apply_sidecar_runtime_policy(status)
        assert engine._manager.entries_blocked_reason == "assistant relay sidecar unhealthy"
        assert status["runtime_policy"]["incident_action"] == "cancel_working_entry_orders"
        assert status["runtime_policy"]["open_position_action"] == "hold_existing_positions"

        healthy = {
            "runtime_policy": engine._sidecar_runtime_policy({
                "running": True,
                "consecutive_send_failures": 0,
                "buffered_event_count": 0,
                "oldest_buffered_event_age_seconds": 0.0,
        ...
        engine._apply_sidecar_runtime_policy(healthy)
        assert engine._manager.entries_blocked_reason == ""

    def test_sidecar_incident_action_cancels_working_entry_orders(self):
        from crypto_trader.live.engine import LiveEngine
        ...
        broker = MagicMock()
        broker.get_open_orders.return_value = [
            SimpleNamespace(order_id="entry-1", tag="entry", metadata={}),
            SimpleNamespace(order_id="stop-1", tag="stop", metadata={}),
        ]
        broker.cancel_order.return_value = True
        engine._slots = [SimpleNamespace(ctx=SimpleNamespace(broker=broker))]
        unhealthy = {
            "running": True,
            "consecutive_send_failures": 3,
            "buffered_event_count": 0,
        ...
        broker.cancel_order.assert_called_once_with("entry-1")
        assert status["runtime_policy"]["incident_action_applied"] is True
        assert status["runtime_policy"]["entry_orders_cancelled"] == 1
