...
class SidecarForwarder:
    ...
    def status(self) -> dict:
        ...
        pending = self._pending_event_stats()
        return {
            "enabled": True,
            ...
            "buffered_event_count": pending["buffered_event_count"],
            "oldest_buffered_event_age_seconds": pending["oldest_buffered_event_age_seconds"],
    ...
    def _pending_event_stats(self) -> dict[str, float | int]:
        count = 0
        oldest_age = 0.0
        now = datetime.now(timezone.utc)
        for _event_type, path, watermark_key in self._event_sources():
            if not path.exists():
                continue
            try:
                size = path.stat().st_size
                ...
                continue
            offset = self._watermarks.get(watermark_key, 0)
            if offset >= size:
                continue
            if offset > size:
                offset = 0
            try:
                ...
                oldest_age = max(oldest_age, (now - modified_at).total_seconds())
            except OSError:
                continue
        return {
            "buffered_event_count": count,
            "oldest_buffered_event_age_seconds": round(max(oldest_age, 0.0), 1),
        }
