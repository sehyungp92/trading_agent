...
from backtests.scalp.analysis.ivb_auction_diagnostics import ivb_auction_diagnostics
...
class IvbAuctionPlugin:
    ...
    ultimate_targets = {"profit_factor": 1.2, "expectancy_dollar": 0.0, "trades_per_month": 4.0, "max_drawdown_pct": 0.15}
    ...
    def _replay_bundle(self) -> ReplayBundle:
        ...
        source_fp = ":".join(
            [
                fingerprint_tree(repo_root / "strategies" / "scalp" / "ivb_auction", patterns=("*.py",)),
        ...
        if self._bundle is None or fingerprint != self._last_fingerprint:
            ...
            self._bundle = ReplayBundle(load_ivb_auction_data(cfg), str(self.data_dir), fingerprint)
    ...
    def run_phase_diagnostics(self, phase: int, state: PhaseState, metrics: dict[str, float], greedy_result) -> str:
        del state
        return f"IVB phase {phase}: score {greedy_result.base_score:.4f}->{greedy_result.final_score:.4f}, trades={metrics.get('total_trades', 0)}"
    ...
    def run_enhanced_diagnostics(self, phase: int, state: PhaseState, metrics: dict[str, float], greedy_result) -> str:
        ...
        return ivb_auction_diagnostics(context["trades"], context["metrics"])
    ...
    def build_end_of_round_artifacts(self, state: PhaseState) -> EndOfRoundArtifacts:
        ...
        return EndOfRoundArtifacts(
            final_diagnostics_text=ivb_auction_diagnostics(context["trades"], context["metrics"]),
            dimension_reports={"ivb_auction": f"Trades={len(context['trades'])}, metrics={context['metrics']}"},
            overall_verdict=f"IVB auction finished with net ${context['metrics'].get('net_profit', 0.0):.2f}.",
        )
    ...
    def _run_config(self, mutations: dict[str, Any]) -> dict[str, Any]:
        ...
        result = run_ivb_auction_backtest(bundle.data, cfg)
