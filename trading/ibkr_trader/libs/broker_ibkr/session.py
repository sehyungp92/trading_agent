...
class UnifiedIBSession:
    ...
    async def verify_streaming_data(
        self,
        test_symbol: str = "SPY",
        *,
        runtime_env: str = "dev",
        ...
        - Live mode fails closed on real-time data errors; delayed fallback is forbidden.
        - Paper mode requires ALLOW_DELAYED_MARKET_DATA=1 before trying delayed data.
        - If both fail, raises RuntimeError with troubleshooting steps.
        - Set env SKIP_STREAMING_CHECK=1 to bypass non-live development checks.
        """
        runtime_env = runtime_env.lower()
        if os.environ.get("SKIP_STREAMING_CHECK", "").lower() in ("1", "true", "yes"):
            if runtime_env == "live":
                raise RuntimeError(
                    "SKIP_STREAMING_CHECK is forbidden in live mode; live trading "
                    "must verify real-time market data availability."
                )
            ...
            return dict(self.market_data_status)
        ...
        # Real-time failed -- try delayed data as fallback only when explicitly allowed.
        rt_code, rt_msg = errors[0]
        if runtime_env == "live":
            raise RuntimeError(
                f"Real-time market data unavailable for {test_symbol} in live mode "
                f"(IBKR error {rt_code}); delayed fallback is forbidden for live trading. "
                f"Raw error: {rt_msg}"
            )
        allow_delayed = os.environ.get("ALLOW_DELAYED_MARKET_DATA", "").lower() in {
            "1",
            "true",
            "yes",
        }
        if runtime_env == "paper" and not allow_delayed:
            raise RuntimeError(
                f"Real-time market data unavailable for {test_symbol} in paper mode "
                f"(IBKR error {rt_code}); set ALLOW_DELAYED_MARKET_DATA=1 to explicitly "
                f"accept delayed paper data. Raw error: {rt_msg}"
            )
        logger.warning(
        ...
        self.market_data_status = {
            ...
            "runtime_env": runtime_env,
            "test_symbol": test_symbol,
            "allow_delayed_market_data": allow_delayed,
            "real_time_error_code": rt_code,
            "real_time_error": rt_msg,
        }
        return dict(self.market_data_status)
