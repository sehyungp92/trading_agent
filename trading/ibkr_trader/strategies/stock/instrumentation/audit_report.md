...
- Shared infrastructure: PostgreSQL and dashboard provided by `trading/ibkr_trader`; relay ingress provided by `packages/trading_assistant`
...
- The relay implementation lives in `packages/trading_assistant/src/trading_assistant/relay_ingress/`.
- The assistant-owned relay forwards relevant events into the local assistant queue.
