"""CLI for the Helix alpha expansion optimizer."""
...
from backtests.swing.auto.helix.alpha_expansion.plugin import HelixAlphaExpansionPlugin
...
def _build_runner(args: argparse.Namespace, *, for_write: bool = True) -> PhaseRunner:
    plugin = HelixAlphaExpansionPlugin(
    ...
    return PhaseRunner(
        ...
        round_name="Helix alpha expansion fast screen with synchronized gates",
...
def cmd_phase_auto(args: argparse.Namespace) -> None:
    ...
    print("Helix alpha expansion auto-optimization complete.")
...
def cmd_phase_gate(args: argparse.Namespace) -> None:
    ...
    phase_mutations = dict(plugin.initial_mutations or {})
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="helix-alpha-expansion",
        description="Helix synchronized alpha expansion phased auto-optimization",
    )
    ...
    phase_run = sub.add_parser("phase-run", help="Run a single Helix alpha expansion phase")
    ...
    phase_auto = sub.add_parser("phase-auto", help="Run all Helix alpha expansion phases")
