...
DEFAULT_BASELINE_MANIFEST = Path("../../deployments/k_stock/generated/live_readiness/olr_kalcb/baseline_manifest.json")
...
def _env_path_default(env_name: str, fallback: Path) -> str:
...
def _prepare_plan(
    ...
    *,
    ...
    health_checks = _with_relay_heartbeat(health_checks, mode=mode)
...
def _with_relay_heartbeat(checks: Mapping[str, Any], *, mode: str) -> dict[str, Any]:
    augmented = dict(checks or {})
    if str(mode or "").lower() not in {"paper", "live"}:
        return augmented
    try:
        ...
        relay_url = os.environ.get("SIDECAR_RELAY_URL") or os.environ.get("RELAY_URL") or ""
        hmac_secret = os.environ.get("INSTRUMENTATION_HMAC_SECRET") or ""
        bot_id = os.environ.get("K_STOCK_SIDECAR_BOT_ID") or "k_stock_trader"
        validation_errors = validate_relay_config(
            relay_url=relay_url,
            hmac_secret=hmac_secret,
            bot_id=bot_id,
            allow_loopback=_allow_loopback_relay(),
            secret_field_name="INSTRUMENTATION_HMAC_SECRET",
        )
        if validation_errors:
            raise ValueError("; ".join(validation_errors))
        effective_hash = _effective_config_hash()
        api_key = bot_exact_ack_api_key(os.environ)
        result = probe_relay_acceptance(
            relay_url=relay_url,
            hmac_secret=hmac_secret,
            bot_id=bot_id,
            runtime_instance_id=f"k-stock:{effective_hash[:12]}",
            effective_config_hash=effective_hash,
            deployment_id=f"k-stock:{effective_hash[:16]}",
            source="k-stock-olr-kalcb-runtime-preflight",
        ...
        if not result.ok:
            raise RuntimeError(result.error or "assistant relay did not accept heartbeat")
        augmented["assistant_relay_accepted"] = {
            "passed": True,
            "detail": (
                f"assistant relay accepted signed heartbeat event_id={result.event_id} "
                f"secret_fingerprint={secret_fingerprint(hmac_secret)}"
            ),
        }
    except Exception as exc:
        augmented["assistant_relay_accepted"] = {
            "passed": False,
            "detail": f"assistant relay heartbeat failed: {exc}",
        }
    return augmented
...
def _effective_config_hash() -> str:
    raw_path = os.environ.get(
        "K_STOCK_EFFECTIVE_CONFIG_PATH",
        "../../deployments/k_stock/generated/olr_kalcb.effective.json",
    )
    try:
        payload = json.loads(_resolve_path(raw_path).read_text(encoding="utf-8"))
    except Exception:
        return "unverified-effective-config"
    return str(
        payload.get("effective_config_hash")
        or payload.get("materialized_config_hash")
...
def _add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--baseline-manifest",
        default=_env_path_default("OLR_KALCB_BASELINE_MANIFEST", DEFAULT_BASELINE_MANIFEST),
        help="Frozen approved optimized-config manifest.",
    )
    parser.add_argument("--strategy-config-source", help="Override strategy config manifest; defaults to --baseline-manifest.")
    parser.add_argument(
        "--portfolio-policy",
        default=_env_path_default("OLR_KALCB_PORTFOLIO_POLICY", DEFAULT_PORTFOLIO_POLICY),
        help="PortfolioPolicyConfig JSON/YAML.",
    )
    parser.add_argument(
        "--sector-map",
        default=_env_path_default("OLR_KALCB_SECTOR_MAP", DEFAULT_SECTOR_MAP),
        help="Approved sector map JSON/YAML.",
    )
