"""VdubusNQ optimization runner: LHS coarse search + Optuna TPE refinement.

Mirrors NQDTCOptimizationRunner but uses VdubusNQ types, 15m data,
and VdubusNQ parameter space.
...
def _evaluate_single(
    ...
    """Evaluate one VdubusNQ parameter set."""
    overrides = vdubus_params_to_overrides(params)
    config = VdubusBacktestConfig(
        ...
        track_signals=False,
        track_shadows=False,
        warmup_daily_es=base_config.warmup_daily_es,
        warmup_1h=base_config.warmup_1h,
        warmup_15m=base_config.warmup_15m,
...
class VdubusOptimizationRunner:
    """Two-stage optimizer for VdubusNQ v4.0 strategy."""
    ...
    def run(self) -> OptimizationResult:
        ...
        logger.info(
            "Starting VdubusNQ optimization: %d coarse + %d refine, %d workers",
    ...
    def _stage_coarse(self) -> list[TrialResult]:
        ...
        samples = latin_hypercube_sample(VDUBUS_PARAM_SPACE, self.n_coarse, self.seed)
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                result = _evaluate_single(sample, self.vdubus_data, self.base_config)
    ...
    def _stage_refine(self, top_results: list[TrialResult]) -> list[TrialResult]:
        ...
        def optuna_objective(trial: optuna.Trial) -> float:
            ...
            try:
                result = _evaluate_single(params, self.vdubus_data, self.base_config)
