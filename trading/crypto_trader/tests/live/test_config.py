...
VALID_RELAY_SECRET = "0123456789abcdef0123456789ABCDEF"
STRATEGY_IDS = ("momentum", "trend", "breakout")
REPO_ROOT = Path(__file__).resolve().parents[4]
CRYPTO_ROOT = REPO_ROOT / "trading" / "crypto_trader"
...
def _valid_live_config_payload(tmp_path: Path) -> dict:
    ...
    return {
        ...
        "relay_secret": VALID_RELAY_SECRET,
        "strategy_configs": {
...
class TestLiveConfig:
    ...
    def test_live_config_example_credentials_do_not_validate(self):
        payload = json.loads(
            (CRYPTO_ROOT / "config/live_config.example.json").read_text(encoding="utf-8")
        )
    ...
    def test_deployment_preflight_accepts_secret_changes_with_same_public_config(self, tmp_path):
        ...
        mounted_payload = {
            **config_payload,
            ...
            "relay_secret": "abcdef0123456789ABCDEF0123456789",
        }
    ...
    def test_deployment_relay_probe_ignores_generic_assistant_read_key(
        self,
        tmp_path,
        monkeypatch,
    ):
        calls = []
        config_payload = _valid_live_config_payload(tmp_path)
        effective_payload = {"effective_config_hash": "b" * 64}
        ...
        monkeypatch.setattr(
            ...
            or SimpleNamespace(ok=True, to_json_dict=lambda: {"ok": True}),
        )

        assert _deployment_relay_probe(config_payload, effective_payload) == {"ok": True}
