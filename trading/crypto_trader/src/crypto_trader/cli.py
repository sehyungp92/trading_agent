...
def paper(config_path: str) -> None:
    ...
    errors = config.validate(require_relay=True)
    if errors:
        for e in errors:
...
def deployment_preflight(config_path: str, effective_config_path: str, runtime_root: str) -> None:
    ...
    relay_probe = None
    if not errors:
        relay_probe = _deployment_relay_probe(config_payload, effective_payload)
        if not relay_probe.get("ok"):
            errors.append(f"assistant relay acceptance failed: {relay_probe.get('error')}")
    result = {"valid": not errors, "errors": errors, "relay_probe": relay_probe}
...
def _deployment_preflight_errors(
    ...
    *,
    ...
    errors.extend(config.validate(require_relay=True))
...
def _deployment_relay_probe(config_payload: dict, effective_payload: dict) -> dict[str, object]:
    from trading_contracts.relay_acceptance import bot_exact_ack_api_key, probe_relay_acceptance

    effective_hash = str(
        effective_payload.get("effective_config_hash")
        or effective_payload.get("materialized_config_hash")
        or _canonical_json_sha256(effective_payload)
    )
    api_key = bot_exact_ack_api_key(os.environ)
    result = probe_relay_acceptance(
        relay_url=str(config_payload.get("relay_url") or ""),
        hmac_secret=str(config_payload.get("relay_secret") or ""),
        bot_id=str(config_payload.get("bot_id") or ""),
        runtime_instance_id=f"crypto-preflight:{effective_hash[:12]}",
        effective_config_hash=effective_hash,
        deployment_id=f"crypto:{effective_hash[:16]}",
        source="crypto-deployment-preflight",
