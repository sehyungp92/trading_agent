...
class BuyAndHoldMetrics:
    """Buy-and-hold benchmark statistics scaled to match strategy sizing."""
    ...
    cagr: float = 0.0
    max_drawdown_pct: float = 0.0
    # Dollar-scaled fields (comparable to strategy)
    qty: int = 1
...
def compute_buy_and_hold(
    ...
    When qty and multiplier are provided, all dollar metrics are scaled
    to match the strategy's capital deployment so comparisons are like-for-like.
    """
    closes = daily_closes[np.isfinite(daily_closes)]
    if len(closes) < 2:
        return BuyAndHoldMetrics(symbol=symbol, initial_equity=initial_equity)

    start_price = float(closes[0])
    end_price = float(closes[-1])
    total_return_pct = (end_price / start_price - 1.0) * 100.0

    # Dollar P&L scaled to qty * multiplier
    scale = qty * multiplier
    bh_pnl = scale * (end_price - start_price)

    # CAGR on the same equity base as the strategy
    if initial_equity > 0 and years > 0:
        cagr = compute_cagr(initial_equity, initial_equity + bh_pnl, years)
    else:
        cagr = 0.0

    # Max drawdown in dollar terms on the equity base
    peak_equity = initial_equity + scale * (closes[0] - start_price)  # == initial_equity
    max_dd_dollar = 0.0
    max_dd_pct = 0.0
    for c in closes:
        equity = initial_equity + scale * (float(c) - start_price)
