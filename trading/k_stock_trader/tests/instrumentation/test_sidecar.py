...
def test_paper_sidecar_rejects_missing_signed_relay_config(tmp_path, monkeypatch):
    monkeypatch.setenv("TRADING_MODE", "paper")
    monkeypatch.delenv("RELAY_URL", raising=False)
    monkeypatch.delenv("SIDECAR_RELAY_URL", raising=False)
    monkeypatch.delenv("INSTRUMENTATION_HMAC_SECRET", raising=False)

    with pytest.raises(ValueError, match="invalid paper/live sidecar relay config"):
        Sidecar(_make_config(tmp_path))
