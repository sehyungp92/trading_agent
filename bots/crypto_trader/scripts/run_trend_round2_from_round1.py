"""Run trend round 2 from the round-1 optimized baseline."""
...
from crypto_trader.optimize.trend_round2_plugin import (
    ROUND2_HARD_REJECTS,
...
ROUND1_CONFIG_PATH = ROOT / "output" / "trend" / "round_1" / "optimized_config.json"
ROUND1_SUMMARY_PATH = ROOT / "output" / "trend" / "round_1" / "round1_summary.json"
DATA_DIR = ROOT / "data"
...
def _score_metrics(metrics: dict[str, float]) -> tuple[float, bool, str]:
    return composite_score(
        metrics,
        weights=ROUND2_SCORING_WEIGHTS,
        hard_rejects=ROUND2_HARD_REJECTS,
        ceilings=ROUND2_IMMUTABLE_SCORING_CEILINGS,
    )
...
def _build_round2_context(
    *,
    ...
    phase_counts = {
        ...
        for phase, generator in ROUND2_PHASE_CANDIDATES.items()
    }
    context = {
        "round": round_num,
        "baseline_config": str(ROUND1_CONFIG_PATH),
        "baseline_round": 1,
        "symbols": SYMBOLS,
        ...
        "scoring_weights": ROUND2_SCORING_WEIGHTS,
        "phase_scoring_emphasis": ROUND2_PHASE_SCORING_EMPHASIS,
        "immutable_scoring_ceilings": ROUND2_IMMUTABLE_SCORING_CEILINGS,
        "hard_rejects": ROUND2_HARD_REJECTS,
        "phase_gate_criteria": {
            str(phase): [criterion.__dict__ for criterion in criteria]
            for phase, criteria in ROUND2_PHASE_GATE_CRITERIA.items()
        },
        "phase_names": ROUND2_PHASE_NAMES,
...
def main() -> None:
    ...
    if not ROUND1_CONFIG_PATH.exists() or not ROUND1_SUMMARY_PATH.exists():
    ...
    baseline_config = TrendConfig.from_dict(_load_json_strategy(ROUND1_CONFIG_PATH))
    round1_summary = json.loads(ROUND1_SUMMARY_PATH.read_text(encoding="utf-8"))
    ...
    runner = PhaseRunner(plugin, round_dir, round_name="trend_round2_alpha_capture")
    ...
    _assert_baseline_matches(round1_summary["final_metrics"], baseline_anchor["metrics"])

    _build_round2_context(
        round_num=round_num,
        round_dir=round_dir,
